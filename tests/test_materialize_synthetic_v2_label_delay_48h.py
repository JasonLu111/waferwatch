...
LABEL_DELAY_12H_LOTS_PATH = (
    ...
    / "scenarios"
    / "LABEL_DELAY_12H"
    / "label_delay_12h_lots.csv"
)
LABEL_DELAY_24H_LOTS_PATH = (
    REPO_ROOT
    / "data"
    / "synthetic"
    / "v2"
...
def test_label_delay_48h_preserves_all_peer_cohorts(
    tmp_path: Path,
) -> None:
    ...
    assert summary.label_delay_hours == 48

    for filename in OUTPUT_FILES.values():
        assert (output_dir / filename).is_file()

    lots = pd.read_csv(output_dir / OUTPUT_FILES["lots"])
    prev_03_lots = pd.read_csv(PREV_03_LOTS_PATH)
    delay_12h_lots = pd.read_csv(LABEL_DELAY_12H_LOTS_PATH)
    delay_24h_lots = pd.read_csv(LABEL_DELAY_24H_LOTS_PATH)

    assert lots["lot_id"].tolist() == prev_03_lots["lot_id"].tolist()
    assert lots["lot_id"].tolist() == delay_12h_lots["lot_id"].tolist()
    assert lots["lot_id"].tolist() == delay_24h_lots["lot_id"].tolist()
    ...
    original_columns = [
        column
        for column in prev_03_lots.columns
        if column != "label_delay_hours"
    ]
    pdt.assert_frame_equal(
        lots[original_columns].reset_index(drop=True),
        prev_03_lots[original_columns].reset_index(drop=True),
        check_dtype=False,
    )
    ...
    assert pd.to_numeric(
        ...
        prev_03_lots["label_delay_hours"],
        errors="raise",
    ).tolist()
...
def test_label_delay_48h_is_byte_reproducible(
    tmp_path: Path,
) -> None:
