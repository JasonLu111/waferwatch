...
def test_label_delay_12h_preserves_prev_03_population(
    tmp_path: Path,
) -> None:
    output_dir = tmp_path / "LABEL_DELAY_12H"
    ...
    assert summary.scenario_id == "LABEL_DELAY_12H"
    ...
    assert summary.label_delay_hours == 12

    for filename in OUTPUT_FILES.values():
        assert (output_dir / filename).is_file()

    lots = pd.read_csv(output_dir / OUTPUT_FILES["lots"])
    source_lots = pd.read_csv(PREV_03_DIR / "prev_03_lots.csv")
...
def test_label_delay_12h_is_byte_reproducible(
    tmp_path: Path,
) -> None:
