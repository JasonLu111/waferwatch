...
## GitHub Reproduction Instructions

This repository is prepared for public GitHub upload as a reproducible portfolio project.

WaferWatch is a cost-sensitive semiconductor fab anomaly monitoring project. It uses the public UCI SECOM-style semiconductor sensor dataset as the data backbone and builds a reproducible machine learning workflow for data ingestion, validation, cleaning, feature preparation, model comparison, thresholding, monitoring reports, and interview-ready documentation.

This repository does not include full raw data, full processed CSV files, model binaries, virtual environments, MLflow tracking folders, or large temporary artifacts.

### Project Overview

WaferWatch focuses on manufacturing AI decision support rather than simple accuracy maximization.

The current repository demonstrates:

- SECOM data ingestion structure
- Data validation and cleaning scripts
- Missing-value handling
- Class imbalance reporting
- Baseline model workflow
- Model comparison reports
- Cost-sensitive and monitoring-oriented documentation
- Model card, data card, and project status documentation

Future phases will add RAG, FastAPI, Docker, Streamlit, MLflow, and deployment-style MLOps components.

### Repository Structure

Expected important folders and files:
...
Data folders are intentionally excluded from GitHub when they contain full raw or processed datasets.
...
The full UCI SECOM dataset is not committed to this repository.

Place the required raw files manually under:
...
These files are ignored by Git to prevent uploading raw data.

The cleaned dataset generated by the pipeline is also ignored by Git:

```text
data/processed/secom_cleaned.csv
```

This is intentional. The goal is to keep the GitHub repository lightweight and reproducible without committing full data files.

### Run Pipeline

After placing the SECOM raw files locally, run:

```powershell
python src/data/ingest_secom.py
python src/data/validate_secom.py
python src/data/clean_secom.py
```
...
Run the existing model and report scripts from the repository root.

Use the scripts currently available under:
...
A clean repository after running experiments should not show raw data, processed CSV files, model binaries, `mlruns/`, or `.venv/` as files to commit.
...
Small sample data may be added in a later phase if needed for a public demo, but full raw data, full processed datasets, model binaries, and experiment tracking folders should not be committed.
...
- Public SECOM-style data is not equivalent to confidential real fab data.
- Full raw data is not committed to GitHub.
- Full processed CSV outputs are generated locally and ignored by Git.
- Synthetic fab metadata and RAG components are planned for later phases.
- Docker, FastAPI, Streamlit, MLflow, and production deployment components will be added in later phases.
- Current results should be interpreted as reproducible portfolio evidence, not real production fab performance.

### Phase R2 GitHub Upload Standard

Before pushing to GitHub, confirm:

```powershell
git status
git ls-files | Select-String -Pattern '^\.venv/|^data/raw/|^data/interim/|^data/processed/.*\.(csv|parquet)$|^data/synthetic/.*\.csv$|^mlruns/|^models/|\.pkl$|\.joblib$|\.onnx$|^\.env$|__pycache__|\.pyc$|\.ipynb_checkpoints'
```

The risky file check should produce no output, except if intentionally tracked small sample files are added in a later phase.
